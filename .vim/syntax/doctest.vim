...
" strings
syn region pythonString		matchgroup=Normal start=+[uU]\='+ end=+'+ skip=+\\\\\|\\'+ contains=pythonEscape,pythonDocTestStart
syn region pythonString		matchgroup=Normal start=+[uU]\="+ end=+"+ skip=+\\\\\|\\"+ contains=pythonEscape,pythonDocTestStart
syn region pythonString		matchgroup=Normal start=+[uU]\="""+ end=+"""+ contains=pythonEscape,pythonDocTestStart
syn region pythonString		matchgroup=Normal start=+[uU]\='''+ end=+'''+ contains=pythonEscape,pythonDocTestStart
syn region pythonRawString	matchgroup=Normal start=+[uU]\=[rR]'+ end=+'+ skip=+\\\\\|\\'+ contains=pythonDocTestStart
syn region pythonRawString	matchgroup=Normal start=+[uU]\=[rR]"+ end=+"+ skip=+\\\\\|\\"+ contains=pythonDocTestStart
syn region pythonRawString	matchgroup=Normal start=+[uU]\=[rR]"""+ end=+"""+ contains=pythonDocTestStart
syn region pythonRawString	matchgroup=Normal start=+[uU]\=[rR]'''+ end=+'''+ contains=pythonDocTestStart
...
syn region pythonDocTestText     start="\%^" end="\%$" contains=pythonDocTest
syn region pythonDocTest         start="^[ \t]*>>>" end="^[ \t]*$" contains=pythonDocTestStart,pythonDocTestResult,pythonStatement,pythonRepeat,pythonConditional,pythonOperator,pythonPreCondit,pythonComment,pythonString,pythonRawString,pythonNumber,pythonBuiltin,pythonException,pythonSpaceError
syn match  pythonDocTestResult   "^\([\t ]*\(\.\.\.\|>>>\)\)\@<!.\+$" contained
syn match  pythonDocTestStart    "^[\t ]*\(\.\.\.\|>>>\)"

hi def  pythonDocTestText        ctermfg=cyan guifg=cyan
hi def  pythonDocTestStart       ctermfg=red guifg=red
