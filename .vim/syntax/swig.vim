...
" Last Change: Thu, 14 Nov 2002 19:26:13 -0200
...
syn cluster	cParenGroup	contains=cParenError,cIncluded,cSpecial,cCommentSkip,cCommentString,cComment2String,@cCommentGroup,cCommentStartError,cUserCont,cUserLabel,cBitField,cCommentSkip,cOctalZero,cCppOut,cCppOut2,cCppSkip,cFormat,cNumber,cFloat,cOctal,cOctalError,cNumbersCom,swigIncluded
...
syn region	swigIncluded	display contained start=+"+ skip=+\\\\\|\\"+ end=+"+
syn region	swigIncluded	display contained start='%\w\+\s*\S'ms=e-1 end="\S*"
syn match	swigIncluded	display contained "<[^>]*>"
syn match	swigInclude	display "^\s*%\(include\|import\)\>\s*["<[:alpha:]]" contains=swigIncluded

syn region	swigIgnored	display contained start='%\w\+\s*\S'ms=e-1 end=";"me=e-1
syn match	swigIgnore	display "^\s*%ignore\>\s*\S\+;" contains=swigIgnored

syn cluster	swigPreProcGroup contains=swigIncluded,swigInclude,swigModule,swigPreProc,swigIgnored,swigIgnore
syn region	swigModule	start="^\s*%module\>" end="$"
syn region	swigPreProc	start="^\s*%\(muttable\>\|immutable\>\)" end="$" keepend
...
if version >= 508 || !exists("did_c_syn_inits")
  ...
  HiLink swigInclude		cInclude
  HiLink swigIncluded		cIncluded
  HiLink swigIgnore		cInclude
  HiLink swigIgnored		cIncluded
  HiLink swigModule		PreProc
  HiLink swigPreProc		PreProc
  HiLink cError			Error
